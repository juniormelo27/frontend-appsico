...
import acceptCookies from '@/libraries/actions/acceptCookies';
import * as ToastPrimitives from "@radix-ui/react-toast";
import Link from 'next/link';


export default function CookiesAccepts({
  cookies: accept,
}: Readonly<{
  cookies: boolean;
}>) {
  ...
  return (
    <ToastProvider>
      <Toast id='cookies-accepts' duration={999999}>
        <ToastClose />
        <div className='flex gap-x-5'>
          <svg
            className='hidden sm:block flex-shrink-0 w-12'
            width='72'
            height='63'
            viewBox='0 0 72 63'
            fill='none'
          ...
          <div className='grow'>
            ...
            <p className='mt-2 text-xs text-gray-600'>
              Ao clicar em {'"'}Permitir todos{'"'}, você concorda com o uso de
              todos os cookies.
            </p>
            <span className=' text-xs text-[0.5rem] text-gray-600'>
              Visite nosso{' '}
              <Link
                className='inline-flex mt-2 text-xs text-[0.5rem] items-center gap-x-1.5 text-primary decoration-2 hover:underline font-medium'
                href='#'
              >
                Política de Cookies
              </Link>{' '}
            ...
            <form action={acceptCookies} className='mt-5 inline-flex gap-x-2'>
              <ToastPrimitives.Close asChild>
                <Button
                  type='submit'
                  size='sm'
                  className='py-2 px-3 rounded font-normal inline-flex items-center gap-x-2 text-sm border border-transparent text-white disabled:opacity-50 disabled:pointer-events-none'
                >
                  Permitir todos
                </Button>
